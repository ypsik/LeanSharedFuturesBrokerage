...
- `GetOpenFuturesOrdersAsync` (JKorf shared client) doesn't route EU X-Perp through the correct OKX instrument type on its own — worked around via `OpenOrdersTradingMode` forcing `TradingMode.DeliveryLinear` for the FUTURES instrument type, so `IsPerpetual()` resolves to `Futures` instead of `Swap` on the underlying REST call.
- `GetPositionsAsync` can return non-futures positions (e.g. margin balances) that aren't part of the traded instrument set; `GetAccountHoldings()` skips any position whose ticker has no SPDB entry rather than handing LEAN a `Symbol` it can't resolve a market-data feed for.
- Client order IDs must be OKX-compliant (max 32 alphanumeric characters) — generated via the shared client's own `GenerateClientOrderId()` rather than the base class's default (longer, hex-prefixed) scheme, which OKX rejects (`51000: Parameter clOrdId error`).
